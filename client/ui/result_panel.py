...
class ResultPanel(QWidget):
    ...
    def __init__(self, initial_dark_mode=False):
        ...
        self.selected_tone = DEFAULT_TONE_PRESET
        self.tone_option_buttons = []
    ...
    def build_ui(self):
        ...
        self.realtime_status_label = QLabel("실시간 모드 중지")
        self.realtime_status_label.setObjectName("inputModeStatusLabel")
        self.realtime_status_label.setFixedHeight(24)
        self.realtime_status_label.setAlignment(Qt.AlignVCenter | Qt.AlignLeft)
        self.realtime_start_btn = QPushButton("실시간 시작")
        self.realtime_start_btn.setObjectName("primaryButton")
        self.realtime_stop_btn = QPushButton("실시간 중지")
        self.realtime_stop_btn.setObjectName("secondaryButton")
        ...
        bottom_status_layout.addWidget(self.realtime_status_label, 1, Qt.AlignVCenter)
        ...
        self.api_enabled_checkbox = QCheckBox("OpenAI API 사용")
        self.api_enabled_checkbox.setObjectName("settingsCheck")
        self.api_reload_btn = QPushButton("API 다시 불러오기")
        self.api_reload_btn.setObjectName("secondaryButton")
        self.api_status_label = QLabel("API 상태 확인 전")
        self.api_status_label.setObjectName("settingsNotice")
    ...
    def _create_spell_tab(self):
        ...
        button_row.addWidget(self.realtime_start_btn)
        button_row.addWidget(self.realtime_stop_btn)
        button_row.addStretch()
    ...
    def _create_tone_tab(self):
        ...
        tone_grid.setContentsMargins(0, 0, 8, 0)
        tone_grid.setHorizontalSpacing(8)
        tone_grid.setVerticalSpacing(8)
        for index, tone in enumerate(TONE_PRESETS):
            tone_grid.addWidget(self._create_tone_option_button(tone), index // 3, index % 3)

        button_row = QHBoxLayout()
        button_row.setContentsMargins(0, 0, 8, 0)
        button_row.setSpacing(10)
        button_row.addStretch()
        button_row.addWidget(self.run_tone_btn)
        button_row.addWidget(self.tone_history_btn)

        layout.addLayout(tone_grid)
        layout.addLayout(button_row)
        layout.addWidget(self.tone_box, 1)
        self.set_selected_tone(self.selected_tone)
        return page

    def _create_tone_option_button(self, tone):
        button = QPushButton(tone)
        button.setObjectName("toneOptionButton")
        button.setCheckable(True)
        button.setToolTip(tone)
        button.clicked.connect(lambda _checked=False, value=tone: self.set_selected_tone(value))
        self.tone_option_buttons.append(button)
        return button

    def set_selected_tone(self, tone):
        self.selected_tone = normalize_tone_preset(tone)
        for button in self.tone_option_buttons:
            button.setChecked(button.text() == self.selected_tone)
    ...
    def _create_settings_tab(self):
        ...
        section.addSpacing(4)
        section.addWidget(self.api_enabled_checkbox)
        api_row = QHBoxLayout()
        api_row.setContentsMargins(34, 0, 0, 0)
        api_row.setSpacing(8)
        api_row.addWidget(self.api_reload_btn)
        api_row.addWidget(self.api_status_label, 1)
        section.addLayout(api_row)
        section.addSpacing(4)
    ...
    def apply_theme(self):
        ...
        self.setStyleSheet(
            ...
            QPushButton#toneOptionButton:hover {{
                background: {colors["button_hover"]};
            }}
            QPushButton#toneOptionButton:checked {{
                background: {colors["accent"]};
                color: {colors["accent_text"]};
                border: 1px solid {colors["accent"]};
            }}
    ...
    def open_settings_tab(self):
        ...
        if not showing_settings:
            ...
            self.api_enabled_checkbox.setChecked(self.saved_api_enabled)
    ...
    def set_api_enabled_checked(self, enabled):
        checked = bool(enabled)
        self.saved_api_enabled = checked
        self.api_enabled_checkbox.setChecked(checked)

    def get_api_enabled_checked(self):
        return self.api_enabled_checkbox.isChecked()

    def set_api_status(self, message, ok=None):
        text = str(message or "").strip() or "API 상태 확인 전"
        self.api_status_label.setText(text)
        if ok is None:
            color = "#7b6658"
        else:
            color = "#2f7d4f" if ok else "#a04735"
        self.api_status_label.setStyleSheet(f"color: {color}; font-weight: 800;")
    ...
    def set_realtime_running(self, running, message=""):
        running = bool(running)
        self.realtime_start_btn.setEnabled(not running)
        self.realtime_stop_btn.setEnabled(running)
        self.realtime_status_label.setText(message or ("실시간 모드 실행 중" if running else "실시간 모드 중지"))
