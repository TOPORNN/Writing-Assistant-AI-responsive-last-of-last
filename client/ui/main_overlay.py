...
class MainOverlay(QWidget):
    ...
    def expand(self):
        ...
        for child in self.card.children():
            if isinstance(child, QWidget) and child not in transient_widgets:
                child.show()
        self.hide_busy()
        self.hide_status()
        self.close_settings_cover()
    ...
    def _show_for_window(self, window_handle=None):
        ...
        manual_position = self._manual_position(window_handle, rect, screen)
        if rect is not None:
            left, top, right, bottom = rect
            margin = 20
            if self._should_hide_for_bounds(left, top, right, bottom, margin):
                ...
                return
            x, y = manual_position or self._responsive_position(left, top, right, bottom, margin)
            self.move(x, y)
        elif screen is not None:
            available = screen.availableGeometry()
            left, top, right, bottom = available.left(), available.top(), available.right(), available.bottom()
            x, y = manual_position or self._responsive_position(left, top, right, bottom, 20)
            self.move(x, y)
        self.show()
        self.raise_()
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton and self._can_start_drag(event.pos()):
            self._dragging = True
    ...
    def mouseReleaseEvent(self, event):
        if self._dragging and event.button() == Qt.LeftButton:
            self._dragging = False
            self._drag_offset = None
            self._remember_manual_position()
            event.accept()
            return
        super().mouseReleaseEvent(event)

    def _can_start_drag(self, position):
        widget = self.childAt(position)
        while widget is not None and widget is not self:
            if isinstance(widget, (QPushButton, QRadioButton, QCheckBox, QTextEdit)):
                return False
            widget = widget.parentWidget()
        return True

    def _manual_position_key(self, window_handle=None):
        handle = int(window_handle or self._last_window_handle or 0)
        if win32gui is not None and handle:
            try:
                handle = int(win32gui.GetAncestor(handle, 2) or handle)
            except Exception:
                pass
        state = "collapsed" if self._collapsed else "expanded"
        return (handle, self._last_reader_name or "", state)

    def _manual_position(self, window_handle, rect, screen):
        saved = self._manual_positions.get(self._manual_position_key(window_handle))
        if not saved:
            return None
        mode, x_value, y_value = saved
        if mode == "target" and rect is not None:
            left, top, right, bottom = rect
            return self._bounded_position(left + x_value, top + y_value, left, top, right, bottom, 20)
        if mode == "screen" and screen is not None:
            available = screen.availableGeometry()
            return self._bounded_position(
                x_value,
                y_value,
                available.left(),
                available.top(),
    ...
    def _remember_manual_position(self):
        rect = self._target_rect(self._last_window_handle)
        key = self._manual_position_key(self._last_window_handle)
        if rect is not None:
            left, top, right, bottom = rect
            x, y = self._bounded_position(self.x(), self.y(), left, top, right, bottom, 20)
            self.move(x, y)
            self._manual_positions[key] = ("target", x - left, y - top)
            return
        screen = QApplication.screenAt(self.frameGeometry().center()) or QApplication.primaryScreen()
        if screen is None:
            self._manual_positions[key] = ("screen", self.x(), self.y())
            return
        available = screen.availableGeometry()
        ...
        self.move(x, y)
        self._manual_positions[key] = ("screen", x, y)

    def _bounded_position(self, x, y, left, top, right, bottom, margin):
        max_x = max(left + margin, right - self.width() - margin)
        max_y = max(top + margin, bottom - self.height() - margin)
        bounded_x = min(max(int(x), left + margin), max_x)
        bounded_y = min(max(int(y), top + margin), max_y)
    ...
    def _handle_score_reason_requested(self):
        if self._should_delegate_reason_to_panel():
            ...
            return
        if self._evaluation_reason:
            self.reason_overlay.show_for_window(self._last_window_handle, self._evaluation_reason)
